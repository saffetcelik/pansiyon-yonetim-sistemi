using Microsoft.EntityFrameworkCore.Migrations;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace PansiyonYonetimSistemi.API.Migrations
{
    /// <inheritdoc />
    public partial class EnsureSchemaFixes : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Bu migration idempotent'tir - önceki migration'ın başarısız olmuş olabileceği
            // durumlar için IF NOT EXISTS guard'larıyla çalışır.
            migrationBuilder.Sql(@"
                ALTER TABLE ""Customers"" ADD COLUMN IF NOT EXISTS ""Notes"" text;

                CREATE TABLE IF NOT EXISTS ""SystemSettings"" (
                    ""Id"" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    ""Key"" character varying(100) NOT NULL,
                    ""Value"" text NOT NULL,
                    ""Description"" character varying(250) NULL,
                    ""UpdatedAt"" timestamp without time zone NOT NULL DEFAULT CURRENT_TIMESTAMP
                );

                CREATE UNIQUE INDEX IF NOT EXISTS ""IX_SystemSettings_Key"" ON ""SystemSettings"" (""Key"");
            ");
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(name: "SystemSettings");
            migrationBuilder.DropColumn(name: "Notes", table: "Customers");
        }
    }
}
